package org.jb.cce.psi

import com.intellij.openapi.application.ApplicationManager
import com.intellij.openapi.project.Project
import com.intellij.openapi.vfs.VirtualFile
import com.intellij.psi.PsiFile
import com.intellij.psi.PsiManager
import org.jb.cce.psi.exceptions.PsiConverterException
import org.jb.cce.uast.Language
import org.jb.cce.uast.FileNode

class PsiConverter {
    fun convert(file: VirtualFile, project: Project, language: Language): FileNode {
        val psi = ApplicationManager.getApplication().runReadAction<PsiFile> {
            PsiManager.getInstance(project).findFile(file)
        }
        return when (language) {
            Language.PYTHON -> {
                if (psi == null) throw PsiConverterException("Cannot get PSI of file")
                val pythonVisitor = PsiPythonVisitor()
                psi.accept(pythonVisitor)
                pythonVisitor.file
            }
            else -> throw PsiConverterException("Unsupported language")
        }
    }
}