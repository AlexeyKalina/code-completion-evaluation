...
class GenerateReportAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val storage = mutableListOf<SessionsStorage>()

        for (configFile in files) {
            val config = serializer.deserializeConfig(VfsUtil.loadText(configFile))
            storage.add(SessionsStorage(configFile.parent.path, config.evaluationType))
        }
        ...
        generateReportUnderProgress(workspace, storage, null, project, false)
    }
